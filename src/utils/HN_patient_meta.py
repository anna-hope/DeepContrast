...
def pat_meta_info(pro_data_dir):
    ...
    for name, data in zip(names, datas):
        pat_ids = []
        artifacts = []
        for patientid, artifact, note in zip(
            df["Patient ID"], df["Artifact-OP"], df["Notes"]
        ):
            ## use consistent ID
        ...
        print(
            "data with artifact:",
            df_af.loc[df_af["Artifact-OP"].isin(["Bad", "Yes", "Minimal"])].shape[0],
        )
    ...
    for name, con in zip(["val", "test"], [conss[1], conss[2]]):
        ...
        for cm0, cm_type in zip([cm, cm_norm], ["raw", "norm"]):
            plot_cm(cm0=cm0, cm_type=cm_type, level=name, save_dir=save_dir)
